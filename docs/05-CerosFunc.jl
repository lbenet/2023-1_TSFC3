...
# ii. Si $f(c)$ tiene el mismo signo que $f(a_0)$ entonces definimos $a_1=c$ y $b_1=b_0$. Si $f(c)$ tiene el mismo signo que $f(b_0)$, entonces definimos $a_1=a_0$ y $b_1=c$. Con estos valores, se vuelve al inicio del método.
...
máximo de cada intervalo candidato a contener ceros. `nmax` es el número de
iteraciones máximo, y sirve para asegurarnos que el ciclo `while` termina.
"""
function ceros_biseccion(f, dom::Interval; tol::AbstractFloat=1/1024, nmax::Int = 15)
    bz = !(0 ∈ f(dom))
    v_candidatos = [dom]  # Este vector incluirá al resultado

    #Bifurca sucesivamente hasta que se cumpla la condición de salida
    its = 0 # número de iteraciones
    while !bz && its ≤ nmax
...
# La raíz 0 está incluida en dos intervalos, en `raices[4]` y `raices[5]`, precisamente en uno de los bordes. Esto se debe a que el dominio inicial usado es simétrico respecto a 0, y además $f(0)=0$. Entonces, si uno rehace el ejercicio, con un dominio ligeramente asimétrico, las 8 raíces aparecen con claridad.
...
#-
# Si las hipótesis del teorema no se cumplen, en particular la condición sobre la cercanía entre el valor inicial y la raíz es muy importante y no es sencillo verificarla, la secuencia entonces puede no converger. Esto puede ocurrir cuando los iterados forman una órbita periódica, o simplemente si saltan de una manera errática.
...
#-
# Igual que para el método usual de Newton, supondremos que $f:[x]\to\mathbb{R}$ es continuamente diferenciable en $[x]$, que $x^*\in[x]$ satisface $f(x^*)=0$, que existe y está bien definida la extensión intervalar de $f^\prime(x)$, y que $0\notin F^\prime([x])$. Las hipótesis que hemos hecho permiten usar el Teorema del Valor Medio, y entonces escribimos
# $$f(x) = f(x^*)+f^\prime(\zeta)(x-x^*),$$
# para alguna $\zeta$ entre $x$ y $x^*$, es decir, que está contenida en $[x]$. De esta expresión, despejamos $x^*$,
# $$
# x^* = x - \frac{f(x)}{f^\prime(\zeta)} \in x - \frac{f(x)}{F^\prime([x])} \equiv N([x]; x).
# $$
# Esta expresión demuestra en particular que $x^*\in N([x],x)$, y dado que $x^*\in[x]$, entonces tenemos además que $x^*\in N([x]; x) \cap [x]$. La cota obtenida al considerar $x=m=\textrm{mid}([x])$
...
#-
# La división extendida devuelve en general dos intervalos, uno de los cuales puede ser vacío. Entonces, el método de Newton intervalar extendido consiste en el método de Newton intervalar aplicado a cada uno de los intervalos producidos por la división extendida, en cada iteración del método.
...
#-
# Como se observa en la figura anterior, la división extendida produce dos intervalos disconexos que se extienden a infinito. Sin embargo, gracias a la intersección con el intervalo inicial, los dos intervalos que resultan $[x_1^{(a)}]$ y $[x_1^{(b)}]$ contienen a *todas* las raíces de la función $f(x)$.

#-
# Esto ilustra cómo el método de Newton intervalar extendido permite desechar intervalos que no tienen raíces de la función $f(x)$, y cómo es posible obtener todos intervalos que contienen a *todas* las raíces dentro del intervalo $[x]$. Esta última observación ilustra un método más general y con muchas aplicaciones: *branch-and-prune*.
